        ...
        for category in categories:
            for page in range(0, 20):
                ...
                for item in jobs_raw:
                    ...
                    location = ", ".join(
                        (loc.get("name") if isinstance(loc, dict) else str(loc)) for loc in locs
                    ) or "Remote"
                    contents = item.get("contents") or ""
                    text = html_to_text(contents) if "<" in contents else contents
                    levels = item.get("levels") or []
                    level_hint = " ".join(
                        (lv.get("name") if isinstance(lv, dict) else str(lv)) for lv in levels
                    )
                    ...
                    results.append(
                        NormalizedJob(
                            ...
                            career_stage=infer_career_stage(f"{title} {level_hint}", text),
